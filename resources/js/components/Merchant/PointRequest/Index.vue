<template>
	<custom-card title="Point Request Log" :url="'/merchant/point-request/create'" icon="fas fa-plus" text="Create New Request">
				...
				<tbody>
					<template v-if="Object.keys(logs).length">
						<tr v-for="(log,i) in logs.data" :key="i">
							...
							<td>
								<router-link :to="{ name: 'merchant.point-request.view', params: { custom: log.custom } }" class="btn btn-primary btn-sm"><i class="fas fa-eye"></i> View</router-link>
							</td>
						</tr>
					</template>
					<template v-else>
...
<script>
	...
	export default {
		...
		methods: {
			loadLogs() {
				axios.get('/api/point-request').then((res) => {
					this.logs = res.data;
				}).catch((error => console.log(error)));
			},
			async pagination(page = 1) {
				await axios.get(`/api/point-request?page=${page}`).then((res) => {
					this.logs = res.data;
				}).catch((error) => console.log(error))
			}
		},
