<template>
	<div class="card card-outline card-primary">
		<div class="card-body">
			...
			<form @submit.prevent="resetPassword" method="post" @keydown="form.onKeydown($event)">
				...
				<div class="row">
					<div class="col-12">
						<button type="submit" :disabled="form.busy" class="btn btn-primary btn-block">Update password</button>
					</div>
					<!-- /.col -->
				</div>
			</form>
...
<script>
	...
	export default {
		...
		methods: {
			resetPassword() {
				this.loading = true;
				this.form.post("/api/auth/reset-password")
					.then((result) => {
						this.loading = false;
						this.success = true;
						this.failure = false;
